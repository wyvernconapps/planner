...
const app=new Function(js+'\n;return {set view(v){view=v},render,setLevel,'+
  'get EV(){return EV},set breakOn(v){breakOn=v},get breaks(){return breaks},'+
  'set hidePast(v){hidePast=v},'+
  'get hidePast(){return hidePast},planTrips,get picks(){return picks}};')();
setTimeout(()=>{
  ...
  console.log('default pet-break windows (the migrated dog runs):');
  console.log('   breakfast',app.breaks[0].from/60+'h to '+app.breaks[0].to/60+'h');
  console.log('   dinner   ',app.breaks[1].from/60+'h to '+app.breaks[1].to/60+'h');
  ...
  console.log('   first breakfast run departs',clock(plan.find(t=>t.meal.includes('Breakfast')&&!t.impossible).depart),'(should be 6:30am)');
